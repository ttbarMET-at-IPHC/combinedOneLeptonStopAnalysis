...
//label is the name used to store the weights
void AutoMVAForttVAna( string ifilename, TString label, TString outfileName = "MVAForttVAna.root", TCut mycut = "", TString myMethodList = "" ) 
{
   ...
   while(!ifile.eof()){
	...
	if(var!=string("") && var!=string(" ")){
		variables.push_back(make_pair(var,type));
   		cout<<"Add variable: "<<var<<"\t Type = "<<type<<endl;
   	}
   }
   cout<<"#############################################"<<endl;
   ...
   TMVA::Factory *factory = new TMVA::Factory( label, outputFile, 
...
//    factory->AddVariable( "var4",                "Variable 4", "units", 'F' );
   ...
   */
   ...
   for(unsigned int i=0;i<variables.size();i++){
   	TString v(variables[i].first.c_str());
	char t = variables[i].second.c_str()[0];
   	//factory->AddVariable(TString(variables[i].first.c_str()),variables[i].second.c_str()); 
   ...
   */
   ...
   if (ReadDataFromAsciiIFormat) {
    ...
    TFile* f0 = TFile::Open("/grid_mnt/opt__sbg__data__safe1/cms/echabert/ttV/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/Prod_13Mai/proof_ttZ.root");
    TFile* f1 = TFile::Open("/grid_mnt/opt__sbg__data__safe1/cms/echabert/ttV/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/Prod_13Mai/proof_ttW.root");
    TFile* f2 = TFile::Open("/grid_mnt/opt__sbg__data__safe1/cms/echabert/ttV/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/Prod_13Mai/proof_tt-dilepton.root");
    TFile* f3 = TFile::Open("/grid_mnt/opt__sbg__data__safe1/cms/echabert/ttV/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/Prod_13Mai/proof_DY4jets.root");

    TTree *signal     = (TTree*)f0->Get("theTree2");
    TTree *signal2     = (TTree*)f1->Get("theTree2");
    TTree *background = (TTree*)f2->Get("theTree2");
    TTree *background2 = (TTree*)f3->Get("theTree2");
   ...
   //mycuts = "Channel != 2 && NBjets >=2 && MET <50 && Mll>=80 && Mll<=100";
