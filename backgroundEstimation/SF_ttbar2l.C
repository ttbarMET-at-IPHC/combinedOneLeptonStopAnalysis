...
int main (int argc, char *argv[])
{
    ...
    vector<string> CR4TagList;

    //CR4TagList.push_back("CR4_2j");
    //CR4TagList.push_back("CR4_3j");
    CR4TagList.push_back("CR4_4j");
    
    //CR4TagList.push_back("CR4_4j_100evts");
    //CR4TagList.push_back("CR4_4j_150evts");
    ...
    for (unsigned int i = 0 ; i < CR4TagList.size() ; i++)
    {
    	vector<Table> tables;
    	for (unsigned int j = 0 ; j < signalRegionsTagList.size() ; j++)
    	{
    ...
    vector<TH1F*> CR4plots(CR4TagList.size());
    for (unsigned int i = 0 ; i < CRTagList.size() ; i++)
    {
    ...
    for (unsigned int i = 0 ; i < CR4TagList.size() ; i++)
    {
    	string hname = "h_"+CR4TagList[i]+"_tail";
    	CR4plots[i] = new TH1F(hname.c_str(),"",signalRegionsTagList.size(),0,signalRegionsTagList.size());
    	for(unsigned int j=0;j<signalRegionsTagList.size();j++){
		CR4plots[i]->GetXaxis()->SetBinLabel(j+1,signalRegionsTagList[j].c_str());
	}
	CR4plots[i]->SetLineColor(CRColorList[i]);
    	CR4plots[i]->SetLineWidth(2);
	CR4plots[i]->SetMarkerStyle(CRMarkerStyle[i]);
	CR4plots[i]->SetMarkerSize(1.5);
    }
    ...
    for (unsigned int i = 0 ; i < CR4TagList.size() ; i++) 
    {
    	//cout<<CR4plots[i]->GetEntries()<<endl;
    	if(i==0) CR4plots[i]->Draw();
    	else	 CR4plots[i]->Draw("same");
    	legCR4.AddEntry(CR4plots[i],CR4TagList[i].c_str(),"lp");
    }
    legCR4.Draw("same");
    cR4.Write();
