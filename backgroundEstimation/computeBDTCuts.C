...
bool goesInDileptonCR(){ return goesInDileptonControlRegion();}
...
int main (int argc, char *argv[])
{
  ...
  // ####################
     ...
     screwdriver.AddRegion("2lepton",     ";2 leptons  (no MT cut)",                  &goesInDileptonCR );
  ...
  // ########################################
    ...
    screwdriver.ApplyScaleFactor("ttbar_2l",    "2lepton",             "doubleLepton", SF_2l);
    screwdriver.ApplyScaleFactor("1ltop",       "2lepton",             "doubleLepton", SF_2l);
    screwdriver.ApplyScaleFactor("W+jets",      "2lepton",             "doubleLepton", SF_2l);
    */
...
//  			 "BDT11","BDT12","BDT13","BDT14","BDT15","BDT16"};
  ...
  //CR
  int NofInterval = 4;
  dummy.clear(); dummy = {"bin1","bin2","bin3","bin4"};
  Table tableIndepIntCR4(dummy, vars, dummy, vars);
  int MinNofEvts_ = 150;
